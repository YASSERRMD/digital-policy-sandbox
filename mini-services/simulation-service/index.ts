// Digital Policy Sandbox - Simulation Service
// Background processing with Socket.io for real-time progress updates

import { createServer } from 'http';
import { Server } from 'socket.io';

// Import Prisma from the main project
...
// ============================================
// SIMULATION ENGINE (Local implementation)
// ============================================
...
class SimulationEngine {
  ...
  constructor(scenarioId: string, config: SimulationConfig) {
  ...
  async run(progressCallback: (progress: number, stage: string) => void): Promise<SimulationMetrics> {
    const results = this.initializeResults();
    const totalSteps = 5;
    let currentStep = 0;

    // Step 1: Calculate revenue
    progressCallback(0, 'Calculating revenue projections...');
    await this.delay(500);
    this.calculateRevenue(results);
    currentStep++;
    progressCallback((currentStep / totalSteps) * 100, 'Revenue calculated');

    // Step 2: Calculate compliance
    progressCallback((currentStep / totalSteps) * 100, 'Calculating compliance rates...');
    await this.delay(500);
    this.calculateCompliance(results);
    currentStep++;
    progressCallback((currentStep / totalSteps) * 100, 'Compliance calculated');

    // Step 3: Calculate workload
    progressCallback((currentStep / totalSteps) * 100, 'Calculating workload impact...');
    await this.delay(500);
    this.calculateWorkload(results);
    currentStep++;
    progressCallback((currentStep / totalSteps) * 100, 'Workload calculated');

    // Step 4: Calculate satisfaction
    progressCallback((currentStep / totalSteps) * 100, 'Calculating satisfaction index...');
    await this.delay(500);
    this.calculateSatisfaction(results);
    currentStep++;
    progressCallback((currentStep / totalSteps) * 100, 'Satisfaction calculated');

    // Step 5: Generate projections
    progressCallback((currentStep / totalSteps) * 100, 'Generating monthly projections...');
    await this.delay(500);
  ...
  private calculateRevenue(results: SimulationMetrics): void {
    ...
    for (const group of this.citizenGroups) {
      ...
      const permitEligible = population * (group.demographics.permitEligibility || 0.3);
      totalPermits += permitEligible * (params.feeAmount || 0);
      
      const avgIncome = group.demographics.averageIncome || 30000;
...
const jobQueue: Map<string, { status: string; progress: number }> = new Map();
const activeSimulations: Map<string, boolean> = new Map();

async function processSimulation(job: SimulationJob): Promise<void> {
  const { simulationId, scenarioId, config } = job;
  ...
  try {
    ...
    // Get population data
    const citizenGroups = await prisma.citizenGroup.findMany({
    ...
    // Initialize engine
    const engine = new SimulationEngine(scenarioId, config);
    ...
    const results = await engine.run((progress, stage) => {
      io.to(`simulation:${simulationId}`).emit('progress', {
        simulationId,
        progress,
        stage,
      });

      jobQueue.set(simulationId, { status: 'running', progress });
    });

    // Save results
    await saveResults(simulationId, scenarioId, results);
    ...
    await prisma.simulation.update({
      ...
      data: {
        ...
        duration: Date.now() - (await prisma.simulation.findUnique({ where: { id: simulationId } }))!.createdAt.getTime(),
      },
    });

    jobQueue.set(simulationId, { status: 'completed', progress: 100 });

    io.to(`simulation:${simulationId}`).emit('complete', {
      simulationId,
      results,
    });
    ...
    jobQueue.set(simulationId, { status: 'failed', progress: 0 });

    io.to(`simulation:${simulationId}`).emit('error', {
      simulationId,
      error: errorMessage,
    });
  } finally {
...
async function saveResults(simulationId: string, scenarioId: string, results: SimulationMetrics): Promise<void> {
  // Save main metrics
  const metrics = [
  ...
  // Save monthly projections as simulation results
...
io.on('connection', (socket) => {
  ...
  socket.on('join:simulation', (simulationId: string) => {
    ...
    const jobStatus = jobQueue.get(simulationId);
    if (jobStatus) {
      socket.emit('status', { simulationId, ...jobStatus });
    }
  });
  ...
  // Start simulation
  socket.on('start:simulation', async (job: SimulationJob) => {
    console.log(`Starting simulation: ${job.simulationId}`);

    // Check if already running
    if (activeSimulations.has(job.simulationId)) {
      socket.emit('error', {
    ...
    activeSimulations.set(job.simulationId, true);
    jobQueue.set(job.simulationId, { status: 'pending', progress: 0 });

    // Process in background
    processSimulation(job).catch(console.error);
  });

  // Get simulation status
  socket.on('status:simulation', (simulationId: string) => {
    const jobStatus = jobQueue.get(simulationId);
    if (jobStatus) {
      socket.emit('status', { simulationId, ...jobStatus });
    }
